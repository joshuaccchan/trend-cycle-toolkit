...
% ---- model_summary --------------------------------------------------------
function testModelSummaryReadsTheNumbersOffThePublishedCSVs(t)
d = summary_fixture();
c = onCleanup(@() rmdir(d, 's'));
rows = model_summary(d);

long = rows(rows.model == "model_a", :);
verifyEqual(t, long.last_quarter, "2025Q4");
verifyEqual(t, long.last_mean, 2.7, 'AbsTol', 1e-12);
verifyEqual(t, long.band, 2, 'AbsTol', 1e-12);
verifyEqual(t, long.sd_change, 0, 'AbsTol', 1e-12);

% A model that ends early reports its own last quarter, not the release's.
short = rows(rows.model == "model_b", :);
verifyEqual(t, short.last_quarter, "2025Q3");

gap = rows(rows.model == "gap_model", :);
verifyEqual(t, gap.deepest, -4, 'AbsTol', 1e-12);
verifyEqual(t, gap.deepest_quarter, "2024Q3");
end


function testModelSummaryReplacesTheGeneratedBlockAndNothingElse(t)
d = summary_fixture();
c = onCleanup(@() rmdir(d, 's'));
doc = fullfile(d, 'guide.md');
write_doc(doc, ['# Guide' newline 'kept above, with a 90% band' newline ...
    '<!-- BEGIN generated by estimates/model_summary.m -->' newline ...
    'numbers from an older vintage' newline ...
    '<!-- END generated -->' newline 'kept below' newline]);

model_summary(d, doc);
txt = fileread(doc);
verifySubstring(t, txt, 'kept above, with a 90% band');
verifySubstring(t, txt, 'kept below');
verifySubstring(t, txt, '| `model_a` | 2025Q4 | 2.70 |');
verifyEmpty(t, strfind(txt, 'numbers from an older vintage'));
end


function testModelSummaryRefusesADocumentWithoutTheMarkers(t)
% A guide that quietly kept an older vintage's numbers would be worse than one
% that failed the release.
d = summary_fixture();
c = onCleanup(@() rmdir(d, 's'));
doc = fullfile(d, 'guide.md');
write_doc(doc, ['# Guide' newline 'no markers here' newline]);
verifyError(t, @() model_summary(d, doc), 'uc:estimates:model_summary:noBlock');
end


% ---------------------------------------------------------------------------
function d = summary_fixture()
% Two trend inflation models, one of them a quarter short, and one gap model,
% written as a release writes them.
d = tempname;
mkdir(d);
dates = (datetime(2024, 1, 1) + calquarters(0:7))';
write_tidy(fullfile(d, 'trend_inflation.csv'), ...
    {'model_a', dates, 2 + (0:7)'/10; 'model_b', dates(1:7), 3 * ones(7, 1)});
write_tidy(fullfile(d, 'output_gap.csv'), ...
    {'gap_model', dates, [0; -1; -4; -2; -1; 0; 1; 0.5]});
end


function write_tidy(f, blocks)
% The tidy shape publish writes: date, model, measure, value, with the band one
% point either side of the mean.
t = table();
for k = 1:size(blocks, 1)
    ...
    for m = ["mean" "p05" "p95"]
        off = 0;
        if m == "p05", off = -1; elseif m == "p95", off = 1; end
        t = [t; table(d, repmat(string(blocks{k, 1}), n, 1), repmat(m, n, 1), ...
            v + off, 'VariableNames', {'date', 'model', 'measure', 'value'})]; %#ok<AGROW>
    end
end
writetable(t, f);
end
