...
For 2026Q1, the most recent quarter all four cover, they give 1.99, 2.73, 3.35 and 4.23 percent.
...
at 0.01, so the trend turns slowly and a sharp movement stays in the cycle. The 2020Q2 estimates
show the size of the difference: `uc_2m` puts the gap at -10.1 percent and `ucur_break2` at -1.9
percent. The same mechanism makes the `uc_2m` gap the more variable of the two over the whole
sample.
...
## Estimates as of Vintage 2026Q2

The numbers in this section describe the current release. Everything above describes the models
and does not change between releases.

Trend inflation, in annualized percent. The band is the width of the 90 percent credible interval
at the model's last quarter, and the last column is the standard deviation of the quarterly
changes in the posterior mean over the whole sample.
...
The output gap, in percent of trend output.

| Model | 2026Q2 | Band width | 2020Q2 | SD over the sample |
|---|---|---|---|---|
| `uc_2m` | 0.14 | 10.75 | -10.08 | 2.54 |
| `ucur_break2` | 0.16 | 5.03 | -1.95 | 0.63 |

The two gap estimates agree about the level of the current gap, which carries no implication of
