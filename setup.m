...
if isempty(ver('stats'))
    warning('uc:setup:noStats', ...
        ['The Statistics and Machine Learning Toolbox is not available. ' ...
         'The four trend inflation models need it, through gamrnd and ' ...
         'normcdf, as do the auxiliary-mixture samplers in uc.sv, through ' ...
...
if isempty(ver('optim'))
    warning('uc:setup:noOptim', ...
        ['The Optimization Toolbox is not available. The psi steps of ' ...
         'uc.models.biuc_lrexp and uc.models.uc_ma maximize their conditionals ' ...
         'with fminsearch and then fminunc, and fminunc is the toolbox ' ...
         'function. A release without it loses those two of the four trend ' ...
         'inflation columns; the other four models run unaffected.']);
end
...
if isfile(fullfile(uc_root, 'run_release.m'))
    fprintf(['  release run:  run_release      (three series from six ' ...
             'models, quarterly)\n']);
else
